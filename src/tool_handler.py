...
class ToolHandler:
    ...
    async def _handle_search_pubmed(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            date_range = DateRange(arguments.get("date_range")) if arguments.get("date_range") else None
            ...
            if search_result.articles:
                ...
                content.append({
                    "type": "text",
                    "text": "No articles found for this query."
                })
    ...
    async def _handle_get_article_details(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not pmids:
                return MCPResponse(
                    content=[{"type": "text", "text": "PMIDs parameter is required"}],
                    is_error=True
                )
            ...
            content.append({
                "type": "text",
                "text": f"**Article Details for {len(articles)} Articles**\n"
            })
            ...
            if articles:
                ...
                content.append({
                    "type": "text",
                    "text": "No articles found for the provided PMIDs."
                })
    ...
    async def _handle_search_by_author(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not author_name:
                return MCPResponse(
                    content=[{"type": "text", "text": "Author name is required"}],
                    is_error=True
                )
    ...
    async def _handle_find_related_articles(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not pmid:
                return MCPResponse(
                    content=[{"type": "text", "text": "PMID is required"}],
                    is_error=True
                )
    ...
    async def _handle_search_mesh_terms(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not term:
                return MCPResponse(
                    content=[{"type": "text", "text": "MeSH term is required"}],
                    is_error=True
                )
    ...
    async def _handle_search_by_journal(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not journal_name:
                return MCPResponse(
                    content=[{"type": "text", "text": "Journal name is required"}],
                    is_error=True
                )
            ...
            for topic, articles in sorted_topics:
                content.append({
                    ...
                    "text": f"\n**{topic}** ({len(articles)} articles)\n" + 
                           "\n".join([f"• {article['title']} (PMID: {article['pmid']})" 
    ...
    async def _handle_compare_articles(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            comparison_fields = arguments.get("comparison_fields", ["authors", "methods", "conclusions"])
            ...
            if len(articles) < 2:
                return MCPResponse(
                    content=[{"type": "text", "text": "Not enough valid articles found for comparison"}],
                    is_error=True
                )
            ...
            content.append({
                "type": "text",
                "text": f"**Comparison of {len(articles)} Articles**\n"
            })
            
            # Basic comparison
            comparison_text = "**Articles:**\n"
            for i, article in enumerate(articles, 1):
                authors_str = format_authors([f"{a.first_name or a.initials} {a.last_name}" 
        ...
        try:
            ...
            if search_result.articles:
                ...
                if article_types:
                    ...
                    for article_type, count in sorted(article_types.items(), key=lambda x: x[1], reverse=True)[:5]:
    ...
    async def _handle_advanced_search(self, arguments: Dict[str, Any]) -> MCPResponse:
        ...
        try:
            ...
            if not search_terms:
                return MCPResponse(
                    content=[{"type": "text", "text": "Search terms are required"}],
                    is_error=True
                )
    ...
    def _format_article_summary(self, article_data: Union[Dict[str, Any], 'Article'], index: int, highlight_mesh: str = None) -> str:
        ...
        if authors:
            ...
            for author in authors[:3]:  # Show first 3 authors
                if isinstance(author, dict):
                    ...
                    first_name = getattr(author, 'first_name', None) or getattr(author, 'initials', '')
    ...
    def _format_article_details(self, article, index: int) -> str:
        ...
        for i, author in enumerate(article.authors[:5], 1):
            ...
            if author.affiliation:
                authors_text += f" ({author.affiliation[:50]}...)" if len(author.affiliation) > 50 else f" ({author.affiliation})"
            authors_text += "\n"
